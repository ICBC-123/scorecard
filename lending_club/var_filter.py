...
def var_filter(dt, y, x=None, iv_limit=0.02, missing_limit=0.95, identical_limit=0.95, var_rm=None, var_kp=None,
        ...
        if len_diff_var_kp > 0:
            warnings.warn(
                "Incorrect inputs; there are {} var_kp variables are not exist in input data, which are removed from var_kp. \n {}".format(
                    len_diff, list(set(var_kp) - set(var_kp2))))
    ...
    dt_var_sel = dt_var_selector.query(
        '(info_value >= {}) & (missingrate <= {}) & (identicalrate <= {})'.format(iv_limit, missing_limit,
                                                                                  identical_limit))
    ...
    if return_rm_reason:
        dt_var_rm = dt_var_selector.query(
            ...
            missingrate=lambda x: ['missingrate>{}'.format(missing_limit) if i else np.nan for i in
                                   (x.missingrate > missing_limit)],
            identicalrate=lambda x: ['identicalrate>{}'.format(identical_limit) if i else np.nan for i in
                                     (x.identicalrate > identical_limit)]
        )
